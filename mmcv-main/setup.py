    ...
    if EXT_TYPE == 'parrots':
        ...
        def _keep_minimal_dcn_op(path):
            ...
            keep = [
                ...
                # 关键：补回 DCN 依赖的公共 CUDA 实现/注册链
                '/common/cuda/deform_conv_cuda_kernel.cuh',
                '/common/cuda/deform_conv_cuda_kernel.cu',
                '/common/cuda/modulated_deform_conv_cuda_kernel.cuh',
                '/common/cuda/modulated_deform_conv_cuda_kernel.cu',
            ]
        ...
        if os.getenv('MMCV_WITH_DIOPI', '0') == '1':
            ...
            all_op_files = (
                    ...
                    glob.glob('./mmcv/ops/csrc/common/cuda/*.cu')
            )

            op_files = [p for p in all_op_files if _keep_minimal_dcn_op(p)]

            print('=' * 80)
            print('Using minimal MMCV DCN op build:')
            for p in op_files:
                print(p)
            print('=' * 80)
